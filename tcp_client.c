...
void log_event(const char *direction, const char *action, const char *filename) {
    ...
    // Check if header already exists (first run or empty file)
...
for (int i = 0; i < recent_count; ++i) {
    ...
    fclose(fp);
    log_event("CLIENT -> SERVER", "Sent", filename);
    printf("?? Sent: %s\n", filename);
...
void receive_file(int sockfd) {
    ...
    log_event("SERVER -> CLIENT", "Received", filename);
    printf("?? Received: %s\n", filename);
...
int main() {
    ...
    while (1) {
        ...
        timeout.tv_sec = 2;  // Polling every 2 seconds
        timeout.tv_usec = 0;
        ...
        if (activity == 0) {
            poll_files(WATCH_DIR, sockfd);  // fallback
            continue;
        }
